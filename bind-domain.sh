...
#
# 用法：
#   ./bind-domain.sh <域名>            # 例：./bind-domain.sh aixinhao.cc
#   ./bind-domain.sh <域名> --check    # 只检查当前绑定状态
#
...
for i in $(seq 1 40); do
  ...
  CNAME_NOW=$(gh api "repos/$OWNER/$REPO/pages" --jq '.cname // ""')
  if [[ "$STATE" == "approved" ]]; then ok "域名已通过校验，证书签发中"; break; fi
  printf '   [%02d/40] state=%s cname=%s\n' "$i" "$STATE" "$CNAME_NOW"
